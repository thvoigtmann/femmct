...
lambdaC = 10. # constant time-scale parameter in the White-Metzner model
gammaC = .1 # characteristic strain parameter in the White-Metzner model
...
path = 'IWM_' + str(Nb) + 'x' + str(Na) + '_pressuredriven_dp-' + str(deltap) + '_GInf-' + str(GInf) +'_lambdaC-' + str(lambdaC) + '_muS-' + str(muS)
...
for i in range(Nh):
    ...
    # For Du(t) = 0 (t <= 0), the White-Metzner model simplifies to 1/lambda(t) = 1/lambdaC and hence G(t=0, a) = GInf*exp(-a/lambdaC)
...
for k in range(1, Nt + 1):
    ...
    lambdaInv = Constant(1./lambdaC) + Constant(sqrt(2.)/gammaC)*sqrt(inner(sym(grad(u)), sym(grad(u))))
